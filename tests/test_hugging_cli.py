...
class TestHuggingCli(unittest.TestCase):
    ...
    def setUp(self, mock_load_dataset, mock_sentence_transformer, mock_login):
        ...
        self.mock_dataset.__iter__.return_value = iter([
            {'City': 'New York', 'Nickname': 'The Big Apple', 'Country': 'USA', 'embeddings': np.array([0.1, 0.2, 0.3])}
        ])
        self.mock_dataset.push_to_hub = MagicMock()


        mock_load_dataset.return_value.map.return_value.map.return_value.remove_columns.return_value = self.mock_dataset
    ...
    def test_structure_data(self):
        ...
        self.assertEqual(result, {'combined_text': 'City: New York; Nickname: The Big Apple; Country: USA'})

    def test_batch_embeddings(self):
        self.client.model.encode.return_value = np.array([[0.1, 0.2, 0.3]])
        batch = {'combined_text': ['City: New York; Nickname: The Big Apple; Country: USA']}
    ...
    def test_create_vec_db(self, mock_sqlite_vec, mock_connect):
        ...
        self.client.final_dataset = {'train': [
            {'City': 'New York', 'Nickname': 'The Big Apple', 'Country': 'USA', 'embeddings': [0.1, 0.2, 0.3]}
        ]}
