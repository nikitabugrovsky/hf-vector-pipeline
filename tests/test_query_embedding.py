...
class TestQueryEmbedding(unittest.TestCase):
    ...
    DB_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'city-nicknames-vec.db')
    ID_TO_QUERY = 100
    ...
    def setUp(self):
        ...
        try:
            ...
            cur = self.con.cursor()
            cur.execute("SELECT embeddings FROM vec_city_nicknames WHERE rowid = ?", (self.ID_TO_QUERY,))
            result = cur.fetchone()
    ...
    def test_retrieved_embedding_values(self):
        ...
        np.testing.assert_allclose(self.retrieved_vector[:5], expected_first_five, rtol=1e-5, atol=1e-8)
